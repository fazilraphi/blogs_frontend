...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        // Try to auto-login right after registration
        try {
          ...
          // fall through to redirect to login with next param
        }
        router.push("/login?registered=true&next=/settings/profile?onboarding=1");
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="glass p-8 md:p-12 rounded-3xl w-full max-w-md border border-white/10 relative z-10">
        ...
        <div className="mt-8 text-center text-sm text-gray-500">
          ...
          <Link href="/login" className="text-[var(--color-apple-green)] hover:underline font-medium">
            Login here
          </Link>
        </div>
      </div>
